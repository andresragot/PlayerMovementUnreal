...
UAureiCharacterMovementComponent::FSavedMove_Aurei::FSavedMove_Aurei()
{
...
bool UAureiCharacterMovementComponent::FSavedMove_Aurei::CanCombineWith(const FSavedMovePtr& NewMove, ACharacter* InCharacter, float MaxDelta) const
{
	const FSavedMove_Aurei* NewAureiMove = static_cast<FSavedMove_Aurei*>(NewMove.Get());

	if (Saved_bWantsToSprint != NewAureiMove->Saved_bWantsToSprint)
	{
		return false;
	}

	if (Saved_bWantsToDash != NewAureiMove->Saved_bWantsToDash)
	{
		return false;
	}

	if (Saved_bWallRunIsRight != NewAureiMove->Saved_bWallRunIsRight)
	{
		return false;
	}
...
void UAureiCharacterMovementComponent::FSavedMove_Aurei::Clear()
{
	FSavedMove_Character::Clear();

	Saved_bWantsToSprint = 0;
	Saved_bWantsToDash = 0;
	Saved_bPressedAureiJump = 0;
...
uint8 UAureiCharacterMovementComponent::FSavedMove_Aurei::GetCompressedFlags() const
{
	uint8 Result = FSavedMove_Character::GetCompressedFlags();

	if (Saved_bWantsToSprint) Result |= FLAG_Sprint;
	if (Saved_bWantsToDash) Result |= FLAG_Dash;
	if (Saved_bPressedAureiJump) Result |= FLAG_JumpPressed;

	return Result;
}

void UAureiCharacterMovementComponent::FSavedMove_Aurei::SetMoveFor(ACharacter* C, float InDeltaTime, FVector const& NewAccel, FNetworkPredictionData_Client_Character& ClientData)
{
	FSavedMove_Character::SetMoveFor(C, InDeltaTime, NewAccel, ClientData);

	const UAureiCharacterMovementComponent* CharacterMovement = Cast<UAureiCharacterMovementComponent>(C->GetCharacterMovement());

	Saved_bWantsToSprint = CharacterMovement->Safe_bWantsToSprint;
	Saved_bPrevWantsToCrouch = CharacterMovement->Safe_bPrevWantsToCrouch;
	Saved_bPressedAureiJump = CharacterMovement->AureiCharacterOwner->bPressedAureiJump;
...
void UAureiCharacterMovementComponent::FSavedMove_Aurei::PrepMoveFor(ACharacter* C)
{
	FSavedMove_Character::PrepMoveFor(C);

	UAureiCharacterMovementComponent* CharacterMovement = Cast<UAureiCharacterMovementComponent>(C->GetCharacterMovement());

	CharacterMovement->Safe_bWantsToSprint = Saved_bWantsToSprint;
	CharacterMovement->Safe_bPrevWantsToCrouch = Saved_bPrevWantsToCrouch;
	CharacterMovement->AureiCharacterOwner->bPressedAureiJump = Saved_bPressedAureiJump;
...
UAureiCharacterMovementComponent::FNetworkPredictionData_Client_Aurei::FNetworkPredictionData_Client_Aurei(const UCharacterMovementComponent& ClientMovement)
	: Super(ClientMovement)
{
}

FSavedMovePtr UAureiCharacterMovementComponent::FNetworkPredictionData_Client_Aurei::AllocateNewMove()
{
	return FSavedMovePtr(new FSavedMove_Aurei());
}

#pragma endregion

UAureiCharacterMovementComponent::UAureiCharacterMovementComponent()
{
	NavAgentProps.bCanCrouch = true;
	AureiServerMoveBitWriter.SetAllowResize(true);
}

void UAureiCharacterMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UAureiCharacterMovementComponent::InitializeComponent()
{
	Super::InitializeComponent();

	AureiCharacterOwner = Cast<APlayerCharacter>(GetOwner());
}

// Network
void UAureiCharacterMovementComponent::UpdateFromCompressedFlags(uint8 Flags)
{
	Super::UpdateFromCompressedFlags(Flags);

	Safe_bWantsToSprint = (Flags & FSavedMove_Aurei::FLAG_Sprint) != 0;
	Safe_bWantsToDash = (Flags & FSavedMove_Aurei::FLAG_Dash) != 0;
}

void UAureiCharacterMovementComponent::OnClientCorrectionReceived(FNetworkPredictionData_Client_Character& ClientData,
...
FNetworkPredictionData_Client* UAureiCharacterMovementComponent::GetPredictionData_Client() const
{
	check(PawnOwner != nullptr)

		if (ClientPredictionData == nullptr)
		{
			UAureiCharacterMovementComponent* MutableThis = const_cast<UAureiCharacterMovementComponent*>(this);

			MutableThis->ClientPredictionData = new FNetworkPredictionData_Client_Aurei(*this);
...
// Getters / Helpers
bool UAureiCharacterMovementComponent::IsMovingOnGround() const
{
	return Super::IsMovingOnGround() || IsCustomMovementMode(CMOVE_Slide) || IsCustomMovementMode(CMOVE_Prone);
}
bool UAureiCharacterMovementComponent::CanCrouchInCurrentState() const
{
	return Super::CanCrouchInCurrentState() && IsMovingOnGround();
}
float UAureiCharacterMovementComponent::GetMaxSpeed() const
{
...
float UAureiCharacterMovementComponent::GetMaxBrakingDeceleration() const
{
...
bool UAureiCharacterMovementComponent::CanAttemptJump() const
{
	return Super::CanAttemptJump() || IsWallRunning() || IsHanging() || IsClimbing();
}

bool UAureiCharacterMovementComponent::DoJump(bool bReplayingMoves)
{
	...
	if (Super::DoJump(bReplayingMoves))
	{
		if (bWasWallRunning)
		{
			...
			auto Params = AureiCharacterOwner->GetIgnoreCharacterParams();
			FHitResult WallHit;
...
// Movement Pipeline
void UAureiCharacterMovementComponent::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
	// Slide
	...
	// Try Mantle
	if (AureiCharacterOwner->bPressedAureiJump)
	{
		SLOG("Trying Aureijump")
			if (TryMantle())
			{
				AureiCharacterOwner->StopJumping();
			}
			else if (TryHang())
			{
			...
			else
			{
				SLOG("Failed Mantle, Reverting to jump")
					AureiCharacterOwner->bPressedAureiJump = false;
...
void UAureiCharacterMovementComponent::UpdateCharacterStateAfterMovement(float DeltaSeconds)
{
...
void UAureiCharacterMovementComponent::PhysCustom(float deltaTime, int32 Iterations)
{
...
void UAureiCharacterMovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector& OldLocation, const FVector& OldVelocity)
{
...
// Movement Event
void UAureiCharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
...
bool UAureiCharacterMovementComponent::ServerCheckClientError(float ClientTimeStamp, float DeltaTime,
...
void UAureiCharacterMovementComponent::CallServerMovePacked(const FSavedMove_Character* NewMove, const FSavedMove_Character* PendingMove, const FSavedMove_Character* OldMove)
{
	...
	BitWriterReset.Pop(AureiServerMoveBitWriter);
...

	{
		...
		AureiServerMoveBitWriter.PackageMap = NetConnection ? ToRawPtr(NetConnection->PackageMap) : nullptr;
	}

	if (AureiServerMoveBitWriter.PackageMap == nullptr)
	{
	...
	if (!MoveDataContainer.Serialize(*this, AureiServerMoveBitWriter, AureiServerMoveBitWriter.PackageMap) || AureiServerMoveBitWriter.IsError())
	{
	...
	PackedBits.DataBits.SetNumUninitialized(AureiServerMoveBitWriter.GetNumBits());

	check(PackedBits.DataBits.Num() >= AureiServerMoveBitWriter.GetNumBits());
	FMemory::Memcpy(PackedBits.DataBits.GetData(), AureiServerMoveBitWriter.GetData(), AureiServerMoveBitWriter.GetNumBytes());
...
void UAureiCharacterMovementComponent::EnterSlide(EMovementMode PrevMode, ECustomMovementMode PrevCustomMode)
{
	bWantsToCrouch = true;
...
void UAureiCharacterMovementComponent::ExitSlide()
{
	bWantsToCrouch = false;
	bOrientRotationToMovement = true;
}
bool UAureiCharacterMovementComponent::CanSlide() const
{
	FVector Start = UpdatedComponent->GetComponentLocation();
	FVector End = Start + CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleHalfHeight() * 2.5f * FVector::DownVector;
	FName ProfileName = TEXT("BlockAll");
	bool bValidSurface = GetWorld()->LineTraceTestByProfile(Start, End, ProfileName, AureiCharacterOwner->GetIgnoreCharacterParams());
	bool bEnoughSpeed = Velocity.SizeSquared() > pow(MinSlideSpeed, 2);

	return bValidSurface && bEnoughSpeed;
}

void UAureiCharacterMovementComponent::PhysSlide(float deltaTime, int32 Iterations)
{
...
void UAureiCharacterMovementComponent::Server_EnterProne_Implementation()
{
	Safe_bWantsToProne = true;
}

void UAureiCharacterMovementComponent::EnterProne(EMovementMode PrevMode, ECustomMovementMode PrevCustomMode)
{
	bWantsToCrouch = true;
...
void UAureiCharacterMovementComponent::ExitProne()
{
}

bool UAureiCharacterMovementComponent::CanProne() const
{
	return IsCustomMovementMode(CMOVE_Slide) || IsMovementMode(MOVE_Walking) && IsCrouching();
}

void UAureiCharacterMovementComponent::PhysProne(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
void UAureiCharacterMovementComponent::OnDashCooldownFinished()
{
	Safe_bWantsToDash = true;
}

bool UAureiCharacterMovementComponent::CanDash() const
{
	return IsWalking() && !IsCrouching() || IsFalling();
}

void UAureiCharacterMovementComponent::PerformDash()
{
...
bool UAureiCharacterMovementComponent::TryMantle()
{
...
FVector UAureiCharacterMovementComponent::GetMantleStartLocation(FHitResult FrontHit, FHitResult SurfaceHit, bool bTallMantle) const
{
	float CosWallSteepnessAngle = FrontHit.Normal | FVector::UpVector;
...
bool UAureiCharacterMovementComponent::TryWallRun()
{
	FVector Start = UpdatedComponent->GetComponentLocation();
	...
	// Check Player Height
	if (GetWorld()->LineTraceSingleByProfile(FloorHit, Start, Start + FVector::DownVector * (CapHH()), "BlockAll", Params))
	{
		return false;
	}
...
void UAureiCharacterMovementComponent::PhysWallRun(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || (CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)))
	{
		...
		//Verificar distancia al suelo
		FVector StartDown = UpdatedComponent->GetComponentLocation();
		FVector CastDeltaDown = -UpdatedComponent->GetUpVector() * 5;
		FVector EndDown = StartDown + CastDeltaDown; 
		FHitResult FloorHit;
		GetWorld()->LineTraceSingleByProfile(FloorHit, StartDown, EndDown, "BloackAll", Params);
		...
		//FVector WallClimbDelta = FVector::UpVector * Velocity * timeTick;

		//FVector GravityDelta = FVector::DownVector * GetGravityZ() * timeTick;

		// Apply acceleration
		CalcVelocity(timeTick, 0.f, false, GetMaxBrakingDeceleration());
		Velocity = FVector::VectorPlaneProject(Velocity, WallHit.Normal) /* + WallClimbDelta + GravityDelta*/;
		float TangentAccel = Acceleration.GetSafeNormal() | Velocity.GetSafeNormal2D();
		bool bVelUp = Velocity.Z > 0.f;
		Velocity.Z += GetGravityZ() * WallRunGravityScaleCurve->GetFloatValue(bVelUp ? -TangentAccel : TangentAccel) * timeTick;
...
bool UAureiCharacterMovementComponent::TryHang()
{
	if (!IsMovementMode(MOVE_Falling)) return false;


	FHitResult WallHit;
	if (!GetWorld()->LineTraceSingleByProfile(WallHit, UpdatedComponent->GetComponentLocation(), UpdatedComponent->GetComponentLocation() + UpdatedComponent->GetForwardVector() * 300, "BlockAll", AureiCharacterOwner->GetIgnoreCharacterParams()))
		return false;
	...
	if (!GetWorld()->OverlapMultiByChannel(OverlapResults, ColLoc, ColRot, ECC_WorldStatic, ColBox, AureiCharacterOwner->GetIgnoreCharacterParams()))
		return false;
...
bool UAureiCharacterMovementComponent::TryClimb()
{
	if (!IsFalling()) return false;

	FHitResult SurfHit;
	FVector CapLoc = UpdatedComponent->GetComponentLocation();
	GetWorld()->LineTraceSingleByProfile(SurfHit, CapLoc, CapLoc + UpdatedComponent->GetForwardVector() * ClimbReachDistance, "BlockAll", AureiCharacterOwner->GetIgnoreCharacterParams());
...
void UAureiCharacterMovementComponent::PhysClimb(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
	...
	GetWorld()->LineTraceSingleByProfile(SurfHit, OldLocation, OldLocation + UpdatedComponent->GetForwardVector() * ClimbReachDistance, "BlockAll", AureiCharacterOwner->GetIgnoreCharacterParams());
	GetWorld()->LineTraceSingleByProfile(FloorHit, OldLocation, OldLocation + FVector::DownVector * CapHH() * 1.2f, "BlockAll", AureiCharacterOwner->GetIgnoreCharacterParams());
...
bool UAureiCharacterMovementComponent::IsServer() const
{
	return CharacterOwner->HasAuthority();
}

float UAureiCharacterMovementComponent::CapR() const
{
	return CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleRadius();
}

float UAureiCharacterMovementComponent::CapHH() const
{
...
void UAureiCharacterMovementComponent::SprintPressed()
{
	Safe_bWantsToSprint = true;
}
void UAureiCharacterMovementComponent::SprintReleased()
{
	Safe_bWantsToSprint = false;
}

void UAureiCharacterMovementComponent::CrouchPressed()
{
	bWantsToCrouch = !bWantsToCrouch;
	GetWorld()->GetTimerManager().SetTimer(TimerHandle_EnterProne, this, &UAureiCharacterMovementComponent::OnTryEnterProne, ProneEnterHoldDuration);
}
void UAureiCharacterMovementComponent::CrouchReleased()
{
	GetWorld()->GetTimerManager().ClearTimer(TimerHandle_EnterProne);
}

void UAureiCharacterMovementComponent::DashPressed()
{
	...
	{
		GetWorld()->GetTimerManager().SetTimer(TimerHandle_DashCooldown, this, &UAureiCharacterMovementComponent::OnDashCooldownFinished, DashCooldownDuration - (CurrentTime - DashStartTime));
	}
}
void UAureiCharacterMovementComponent::DashReleased()
{
	GetWorld()->GetTimerManager().ClearTimer(TimerHandle_DashCooldown);
	Safe_bWantsToDash = false;
}

void UAureiCharacterMovementComponent::ClimbPressed()
{
	if (IsFalling() || IsClimbing() || IsHanging()) bWantsToCrouch = true;
}

void UAureiCharacterMovementComponent::ClimbReleased()
{
	bWantsToCrouch = false;
}

bool UAureiCharacterMovementComponent::IsCustomMovementMode(ECustomMovementMode InCustomMovementMode) const
{
	return MovementMode == MOVE_Custom && CustomMovementMode == InCustomMovementMode;
}
bool UAureiCharacterMovementComponent::IsMovementMode(EMovementMode InMovementMode) const
{
...
void UAureiCharacterMovementComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME_CONDITION(UAureiCharacterMovementComponent, Proxy_bDash, COND_SkipOwner)

		DOREPLIFETIME_CONDITION(UAureiCharacterMovementComponent, Proxy_bShortMantle, COND_SkipOwner)
		DOREPLIFETIME_CONDITION(UAureiCharacterMovementComponent, Proxy_bTallMantle, COND_SkipOwner)
}

void UAureiCharacterMovementComponent::OnRep_Dash()
{
	CharacterOwner->PlayAnimMontage(DashMontage);
	DashStartDelegate.Broadcast();
}

void UAureiCharacterMovementComponent::OnRep_ShortMantle()
{
	CharacterOwner->PlayAnimMontage(ProxyShortMantleMontage);
}

void UAureiCharacterMovementComponent::OnRep_TallMantle()
{
